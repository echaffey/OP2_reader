...
def _infer_nrows_per_elem(data: bytes, endian: str = "<") -> Optional[int]:
    ...
    def looks_packed_eid(v: int) -> bool:
        ...
        loc = v % 10
        eid = v // 10
        return 1 <= loc <= 9 and 1 <= eid <= 10_000_000
...
def _decode_solid_payload_extended(
    ...
    Layout per element (NUMWDE words total):
      ...
      w[1 .. centroid_wds]             centroid row (24 words): layout varies,
                                       not decoded (mixed int/float fields)
      w[centroid_wds+1 ..]             corner rows (21 words each):
                                       grid_id + SX,SY,SZ,SXY,SYZ,SZX + more fields

    Only corner rows are emitted (GRID != 0).
    """
    ...
    while offset + stride <= n_words:
        ...
        # Skip centroid row; emit corner rows only.
        # Extended CTETRA corner row layout (21 words per row, including grid_id):
        #   [0]  grid_id
        #   [1]  Normal-X  (SX)
        #   [2]  Shear-XY  (SXY)
        #   [3]  Principal-A
